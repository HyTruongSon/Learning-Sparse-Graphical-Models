// Software: Multi-Layer Perceptron Library (Neural Network) in Java
...
public class MLP_ReLU {
	...
	public MLP_ReLU(int InputLayer, int OutputLayer){
		initMemory(2);
	...
	public MLP_ReLU(int InputLayer, int HiddenLayer, int OutputLayer){
		initMemory(3);
	...
	public MLP_ReLU(int InputLayer, int HiddenLayer1, int HiddenLayer2, int OutputLayer){
		initMemory(4);
	...
	public MLP_ReLU(int InputLayer, int HiddenLayer1, int HiddenLayer2, int HiddenLayer3, int OutputLayer){
		initMemory(5);
	...
	public MLP_ReLU(int InputLayer, int HiddenLayer1, int HiddenLayer2, int HiddenLayer3, int HiddenLayer4, int OutputLayer){
		initMemory(6);
	...
	public void writeWeights(ArrayList<String> FileNames) throws IOException {		
		for (int i = 0; i < nLayers - 1; i++) {
			FileWriter FileOutput = new FileWriter(FileNames.get(i));
        	PrintWriter Writer = new PrintWriter(FileOutput);
	...
	private double ReLU_gradient(double out){
		if (out > 0.0) {
			return 1.0;
		}
		return 0.0;
    ...
    private void initGradients(){
        for (int i = 0; i < Layer[nLayers - 1].n; i++){
			...
			Layer[nLayers - 1].theta[i] = ReLU_gradient(out) * (Expected[i] - out);
		}
		...
		for (int i = 1; i < nLayers - 1; i++)
			for (int j = 0; j < Layer[i].n; j++){
				...
				Layer[i].theta[j] = ReLU_gradient(out) * sum;
			}
    }
